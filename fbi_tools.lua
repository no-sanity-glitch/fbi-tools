...
if enable_autoupdate then
    ...
    if updater_loaded then
        ...
        if autoupdate_loaded then
            Update.json_url = "https://raw.githubusercontent.com/no-sanity-glitch/fbi-tools/refs/heads/main/version.json?" .. tostring(os.clock())
...
function callForBackup()
    ...
    sampAddChatMessage(message, -1) -- change to 'sampSendChat' after debugging
end
...
function issueWanted(attackerId)
    ...
    if name then
        sampAddChatMessage(string.format("/su %d 1.4", attackerId), -1) -- change to 'sampSendChat' after debugging
    end
end

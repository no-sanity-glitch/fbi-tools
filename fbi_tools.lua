...
function main()
    ...
    sampAddChatMessage("{AC0046}[Tools] {FFFFFF}Àêòèâèðîâàí.", -1)
    sampAddChatMessage("{AC0046}[Tools] {FFFFFF}Îòêðûòü ìåíþ - {AC0046}/tt", -1)
end
...
function loadBinds()
    ...
    if not doesFileExist(json) then
        local t = {
            ['backup_text'] = u8("Òðåáóåòñÿ ïîäêðåïëåíèå. Ðàéîí %s."),
            ['cuff_text'] = u8("/do Íàðó÷íèêè íà ïîÿñå."),
            ['fme_text'] = u8("/me âçÿë %s çà áèöåïñ è ïîâåë ïåðåä ñîáîé."),
            ['frisk_text'] = u8("/me îùóïûâàåò ðóêè, íîãè, òîðñ %s, ïðîâåðÿÿ ñîäåðæèìîå êàðìàíîâ."),
            ['incar_text'] = u8("/me îòêðûë äâåðü àâòî, ïîìîã %s ñåñòü, ïðèñòåãíóë ðåìíåì áåçîïàñíîñòè, çàêðûë äâåðü."),
            ['eject_text'] = u8("/me îòêðûë äâåðü àâòî, îòñòåãíóë ðåìåíü áåçîïàñíîñòè, ïîìîã %s âûéòè èç àâòî."),
            ['arest_text'] = u8("/me äîñòàë êëþ÷ îò êàìåðû, îòêðûë äâåðü, çàâåë %s, ñíÿë íàðó÷íèêè, çàõëîïíóë äâåðü."),
        }
        jsonSave(json, t)
    end
    binds = jsonRead(json)
...
function bind_cuff(param)
    ...
    if not id then
        sampAddChatMessage('{CCCCCC}[ Ìûñëè ]: Çàêîâàòü â íàðó÷íèêè [ /cuff ID ]', -1)
        return
    end
...
function bind_follow_me(param)
    local id = tonumber(param)
    if not id then
        sampAddChatMessage('{CCCCCC}[ Ìûñëè ]: Âåñòè çà ñîáîé [ /fme ID ]', -1)
        return
    end
...
function bind_frisk(param)
    local id = tonumber(param)
    if not id then
        sampAddChatMessage('{CCCCCC}[ Ìûñëè ]: Îáûñêàòü èãðîêà [ /frisk ID ]', -1)
        return
    end
...
function bind_incar(param)
    local id = tonumber(param)
    if not id then
        sampAddChatMessage('{CCCCCC}[ Ìûñëè ]: Ïîñàäèòü â ìàøèíó [ /incar ID ]', -1)
        return
    end
...
function bind_eject(param)
    local id = tonumber(param)
    if not id then
        sampAddChatMessage('{CCCCCC}[ Ìûñëè ]: Âûêèíóòü èç òðàíñïîðòà [ /eject ID ]', -1)
        return
    end
...
function bind_arest(param)
    local id = tonumber(param)
    if not id then
        sampAddChatMessage('{CCCCCC}[ Ìûñëè ]: Ïîñàäèòü ïðåñòóïíèêà [ /arest ID ]', -1)
        return
    end
...
function cmdSetWeather(param)
    if param == nil or param == "" then
        sampAddChatMessage("{AC0046}[Tools] {FFFFFF}Èñïîëüçóéòå: /sw [0-45]", 0xFFFFFF)
        return
    end
...
---- Ïðèâåòñòèâå -------
local changelog10 = [[
1. Ñìåíà ñòèëÿ äëÿ ìåíþ.
2. Áèíä êíîïêè îòêðûòèÿ/çàêðûòèÿ ìåíþ.
3. Áèíä êíîïêè îòêðûòèÿ/çàêðûòèÿ battlepass.
4. Äîáàâëåíû ñòðîáîñêîïû. Àêòèâàöèÿ ñèðåíû /strobes | Àêòèâàöèÿ ñòðîáîñêîïîâ P.
5. Äîáàâëåíà ñìåíà ïîãîäû. Êîìàíäà /sw [0-45].
6. Äîáàâëåíî îòêëþ÷åíèå ÎÎÑ ÷àòà. Êîìàíäà /ss.
7. Äîáàâëåíî î÷èñòêà ÷àòà. Êîìàíäà /cc.
8. Äîáàâëåí àâòî ïîèñê ïðåñòóïíèêà.
9. Äîáàâëåíà àâòî ïî÷èíêà è çàïðàâêà â ãîñ. ãàðàæàõ.
10. Äîáàâëåí âûçîâ ïîäêðåïëåíèÿ è ïðåäëîæåíèå î âûäà÷è çâ¸çä ïðè ñòðåëüáå.
11. Äîáàâëåíî àâòîîáíîâëåíèå ñêðèïòà.
12. Äîáàâëåí ÷¸ðíûé ýêðàí äëÿ ññîê. /blackout
13. Äîáàâëåí àâòî ïåðåíîñ â /d è /r.
14. Äîáàâëåíû áàçîâûå îòûãðîâêè /cuff, /fme, /frisk, /incar, /eject, /arest.
]]

local authors = [[

    Ñïàñèáî çà ïîìîùü:
     Dave Grand (êðàñèâàÿ imgui ìåíþøêà, ñòðîáîñêîïû, ñìåíà ïîãîäû);
     Amelia Brown (ðåæèì IC-only ÷àòà è åãî î÷èñòêà).
]]

function welcomeMenu()
    imgui.CenterText('' .. thisScript().name .. u8 ' | v' .. thisScript().version .. ' | Developers - Saburo Arasaka')
    imgui.CenterText(u8 'Ðàçðàáîòàí ñïåöèàëüíî äëÿ Pears Project')
...
function sampev.onSendTakeDamage(playerId, _, _, _, _)
    ...
    if not AutoAlert.attackerCooldowns[playerId] or now - AutoAlert.attackerCooldowns[playerId] >= CONFIG.ALERT_INTERVAL then
        ...
        sampAddChatMessage(
            string.format(
                "{3399FF}Ïî âàì âåä¸ò îãîíü %s. Âûçâàòü ïîäêðåïëåíèå - {FFFF00}%s{3399FF}, îáúÿâèòü â ðîçûñê - {FFFF00}%s",
                name,
...
function cmdFind(arg)
    local id = tonumber(arg)
    if not id then
        sampAddChatMessage("[Tools] {ff0000}Íåâåðíûé{ffffff} ID èãðîêà.", State.main_color)
        return
    end
...
function sampev.onPlayerQuit(playerId, reason)
    if State.auto_find_state.v and AutoFind.state and playerId == AutoFind.targetId then
        ...
        sampAddChatMessage(
            string.format(
                '[Tools] {ffffff}Èãðîê {FF9C00}%s[%d] {FFFFFF}âûøåë ñ ñåðâåðà. {FF9C00}Ïðè÷èíà: {FFFFFF}%s.',
                nickname, playerId, CONFIG.QUIT_REASONS[reason] or 'íåèçâåñòíî'
            ),
            State.main_color
        )
        cmdFindOff()
...
function loadPearsLauncherSkins() -- from vAcs
    ...
    if doesFileExist(file) then
        ...
        print('[Tools] Çàãðóæåíî ' .. l_s_count .. ' ñêèíîâ!')
    else
        print('[Tools] Íå óäàëîñü çàãðóçèòü ñêèíû èç ëàóí÷åðà.', 2, true, false)
    end
end
...
function renderLeftPanel()
    ...
    local menuButtons = {
        ...
        { text = u8 'Áèíäåð', menu = 7 },
        { text = u8 'Ñïåö.Êëàâèøè', menu = 9 },
        { text = u8 'Íàñòðîéêè', menu = 10 }
    }
...
function menu_1()
    imgui.CenterText(u8 'Äîáðî ïîæàëîâàòü, ' .. getMyNick())
    imgui.Separator()
    if imadd.ToggleButton("##straboscopes", State.straboscopes) then
        if State.straboscopes.v then
            sampAddChatMessage("[Tools] {FFFFFF}Ñòðîáîñêîïû {01DF01}âêëþ÷åíû{ffffff}.", State.main_color)
            mainIni.state.stroboscopes = true
            save()
        else
            sampAddChatMessage("[Tools] {FFFFFF}Ñòðîáîñêîïû {ff0000}îòêëþ÷åíû{ffffff}.", State.main_color)
            mainIni.state.stroboscopes = false
            save()
        end
    end
    imgui.SameLine()
    imgui.Text(u8 "Ñòðîáîñêîïû")
    imgui.SameLine()
    imgui.HelpMarker(u8 "Àêòèâàöèÿ ñèðåíû /strobes | Àêòèâàöèÿ ñòðîáîñêîïîâ P")

    if imgui.Button(u8 'Ïåðåçàãðóçèòü ñêðèïò', ImVec2(490, 0)) then
        sampAddChatMessage('[Tools] {FFFFFF}Ïåðåçàãðóçêà...', State.main_color)
        showCursor(false)
        thisScript():reload()
    end
    if imgui.Button(u8 'Âûêëþ÷èòü ñêðèïò', ImVec2(490, 0)) then
        sampAddChatMessage('[Tools] {FFFFFF}Âûêëþ÷àåì ñêðèïò...', State.main_color)
        showCursor(false)
        thisScript():unload()
    end
end
...
function menu_2()
    if imadd.ToggleButton("##auto_alert_state", State.auto_alert_state) then
        sampAddChatMessage(
            "[Tools] {FFFFFF}Çàïðîñ î ïîääåðæêå " ..
            (State.auto_alert_state.v and "{01DF01}âêëþ÷¸í" or "{ff0000}îòêëþ÷¸í") .. "{ffffff}.", State.main_color)
        mainIni.state.auto_alert = State.auto_alert_state.v
        save()
    end
    imgui.SameLine()
    imgui.Text(u8 "Çàïðîñ î ïîääåðæêå")
    -- backup button
    if imgui.HotKey("##HotKeys.backup", HotKeys.backup) then
        ...
        save()
        sampAddChatMessage("[Ïîäñêàçêà] {FFFFFF}Íîâàÿ êëàâèøà íàçíà÷åíà.", State.main_color)
    end
    imgui.SameLine()
    imgui.Text(u8 'Èçìåíèòü êíîïêó çàïðîñà ïîääåðæêè')
    -- su button
    ...
    imgui.SameLine()
    imgui.Text(u8 'Èçìåíèòü êíîïêó âûäà÷è ðîçûñêà')
    -- backup text
    imgui.Text(u8 'Èçìåíèòü òåêñò âûçîâà ïîääåðæêè')
    imgui.SameLine()
    imgui.HelpMarker(u8 "Èñïîëüçóéòå #zone äëÿ óêàçàíèÿ ðàéîíà. Íàïðèìåð: Òðåáóåòñÿ ïîäêðåïëåíèå. Ðàéîí #zone.")
...
function menu_3()
    if imadd.ToggleButton("##auto_find_state", State.auto_find_state) then
        sampAddChatMessage(
            "[Tools] {FFFFFF}Àâòîïîèñê " ..
            (State.auto_find_state.v and "{01DF01}âêëþ÷¸í" or "{ff0000}îòêëþ÷¸í") .. "{ffffff}.", State.main_color)
        mainIni.state.auto_find = State.auto_find_state.v
        save()
    end
    imgui.SameLine()
    imgui.Text(u8 "Àâòîïîèñê")
    imgui.Text(u8 'Àêòèâàöèÿ: /find [id]')
    imgui.Text(u8 'Äåàêòèâàöèÿ: /findoff')
    -- Íóæíà èíôà!!!
    -- imgui.Separator()
    -- imgui.Text(u8 'Óðîâåíü Ñûùèêà')
    -- imgui.SameLine()
    -- imgui.HelpMarker(u8 "/skill - Íàâûê Ñûùèêà")
    -- for i = 10, 1, -1 do
...
function menu_4()
    if imadd.ToggleButton("##auto_fix_state", State.auto_fix_state) then
        sampAddChatMessage(
            "[Tools] {FFFFFF}Àâòîïî÷èíêà " ..
            (State.auto_fix_state.v and "{01DF01}âêëþ÷åíà" or "{ff0000}îòêëþ÷åíà") .. "{ffffff}.", State.main_color)
        mainIni.state.auto_fix = State.auto_fix_state.v
        save()
    end
    imgui.SameLine()
    imgui.Text(u8 "Àâòîïî÷èíêà è àâòîçàïðàâêà")
    imgui.SameLine()
    imgui.HelpMarker(u8 "Àâòîïî÷èíêà è àâòîçàïðàâêà â ãîñ.ãàðàæàõ. Äëÿ ïî÷èíêè íóæíî çàãëóøèòü äâèãàòåëü.")
end

function menu_5()
    imgui.Text(u8 "Âèçóàëüíàÿ ñìåíà ñêèíà")
    imgui.Text(u8 'Ïîèñê:')
    imgui.InputText(u8 '', State.skinSearch)
    if imgui.Selectable(u8 'Âûêë', SkinChanger.selectedSkin == 0) then
...
function menu_6()
    imgui.Text(u8 "/ss - òîëüêî IC ÷àò (óäàëåíèå ad, àäìèíñêèõ ñòðîê, PayDay, /r, /d, OOC ÷àòîâ).")
    imgui.Text(u8 "/ññ - î÷èñòèòü ÷àò.")
    imgui.Text(u8 "/blackout - ÷¸ðíûé ýêðàí.")
end

function menu_7()
    imgui.CenterText(u8 'Áàçîâûå îòûãðîâêè')
    imgui.Separator()
    if imadd.ToggleButton("##binder", State.binder) then
        sampAddChatMessage(
            "[Tools] {FFFFFF}Áèíäåð " ..
            (State.binder.v and "{01DF01}âêëþ÷¸í" or "{ff0000}îòêëþ÷¸í") .. "{ffffff}.", State.main_color)
        mainIni.state.binder = State.binder.v
        save()
    end
    imgui.SameLine()
    imgui.Text(u8 "Áèíäåð")
    imgui.SameLine()
    imgui.HelpMarker(u8 "Âïèøèòå âàøó îòûãðîâêó èëè îñòàâüòå êàê åñòü, #name çàìåíÿåòñÿ íà èìÿ ÷åëîâåêà id êîòîðîãî âû âïèñàëè.")
    if imadd.ToggleButton("##bindcuff", Binder.cuff) then
        sampAddChatMessage(
            "[Tools] {FFFFFF}Îòûãðîâêà íàðó÷íèêîâ " ..
            (Binder.cuff.v and "{01DF01}âêëþ÷åíà" or "{ff0000}îòêëþ÷åíà") .. "{ffffff}.", State.main_color)
        mainIni.state.bind_cuff = Binder.cuff.v
        save()
    end
    imgui.SameLine()
    imgui.Text(u8 "Îòûãðîâêà íàðó÷íèêîâ")
    ...
    if imadd.ToggleButton("##bindfme", Binder.fme) then
        sampAddChatMessage(
            "[Tools] {FFFFFF}Îòûãðîâêà âåñòè çà ñîáîé " ..
            (Binder.fme.v and "{01DF01}âêëþ÷åíà" or "{ff0000}îòêëþ÷åíà") .. "{ffffff}.", State.main_color)
        mainIni.state.bind_fme = Binder.fme.v
        save()
    end
    imgui.SameLine()
    imgui.Text(u8 "Îòûãðîâêà âåñòè çà ñîáîé")
    ...
    if imadd.ToggleButton("##bindfrisk", Binder.frisk) then
        sampAddChatMessage(
            "[Tools] {FFFFFF}Îòûãðîâêà îáûñêà " ..
            (Binder.frisk.v and "{01DF01}âêëþ÷åíà" or "{ff0000}îòêëþ÷åíà") .. "{ffffff}.", State.main_color)
        mainIni.state.bind_frisk = Binder.frisk.v
        save()
    end
    imgui.SameLine()
    imgui.Text(u8 "Îòûãðîâêà îáûñêà")
    ...
    if imadd.ToggleButton("##bindincar", Binder.incar) then
        sampAddChatMessage(
            "[Tools] {FFFFFF}Îòûãðîâêà /incar " ..
            (Binder.incar.v and "{01DF01}âêëþ÷åíà" or "{ff0000}îòêëþ÷åíà") .. "{ffffff}.", State.main_color)
        mainIni.state.bind_incar = Binder.incar.v
        save()
    end
    imgui.SameLine()
    imgui.Text(u8 "Îòûãðîâêà /incar")
    ...
    if imadd.ToggleButton("##bindeject", Binder.eject) then
        sampAddChatMessage(
            "[Tools] {FFFFFF}Îòûãðîâêà /eject " ..
            (Binder.eject.v and "{01DF01}âêëþ÷åíà" or "{ff0000}îòêëþ÷åíà") .. "{ffffff}.", State.main_color)
        mainIni.state.bind_eject = Binder.eject.v
        save()
    end
    imgui.SameLine()
    imgui.Text(u8 "Îòûãðîâêà /eject")
    ...
    if imadd.ToggleButton("##bindarest", Binder.arest) then
        sampAddChatMessage(
            "[Tools] {FFFFFF}Îòûãðîâêà /arest " ..
            (Binder.arest.v and "{01DF01}âêëþ÷åíà" or "{ff0000}îòêëþ÷åíà") .. "{ffffff}.", State.main_color)
        mainIni.state.bind_arest = Binder.arest.v
        save()
    end
    imgui.SameLine()
    imgui.Text(u8 "Îòûãðîâêà /arest")
...
function menu_9()
    ...
    imgui.SameLine()
    imgui.Text(u8 'Èçìåíèòü êíîïêó àêòèâàöèè ìåíþ')
    -- battlepass
    ...
    imgui.SameLine()
    imgui.Text(u8 'Èçìåíèòü êíîïêó àêòèâàöèè Battle Pass')
end

function menu_10()
    local styles = { u8 "Ñåðàÿ", u8 "Êðàñíàÿ", u8 "Ôèîëåòîâàÿ", u8 "×¸ðíàÿ", u8 "Ñèíÿÿ", u8 "Îðàíæåâàÿ", u8 "Ðîçîâàÿ" }
    imgui.Combo(u8 'Ñòèëü èíòåðôåéñà', elements.int.intImGui, styles)
    imgui.Separator()
    -- if imadd.ToggleButton("##idDialog", State.CheckBoxDialogID) then
    --     if State.CheckBoxDialogID.v then
    --         sampAddChatMessage("[Ïîäñêàçêà] {FFFFFF}Dialog ID {01DF01}âêëþ÷¸í{ffffff}.", State.main_color)
    --     else
    --         sampAddChatMessage("[Ïîäñêàçêà] {FFFFFF}Dialog ID {ff0000}îòêëþ÷¸í{ffffff}.", State.main_color)
    --     end
    -- end
    -- imgui.SameLine()
    -- imgui.TextColoredRGB('[Âûêë/Âêë]  {FF0000}Dialog ID')
end
